...
const Game = function Game (channel, client, config, cmdArgs, dbModels) {
  ...
  const questions = _.filter(config.cards, function (card) {
    return card.type.toLowerCase() === 'question';
  });
  console.log(questions.length, 'questions');

  const answers = _.filter(config.cards, function (card) {
  ...
  self.addPlayer = function (player) {
    if (config.gameOptions.database === true && typeof self.dbGame === 'undefined') {
      self.playersToAdd.push(player);
            ...
            // Returning players
      const oldPlayer = _.findWhere(self.players, {nick: player.nick, hostname: player.hostname, isActive: false});
  ...
  self.showPoints = function () {
    const sortedPlayers = _.sortBy(self.players, function (player) {
  ...
  self.showStatus = function () {
    let  // amount of player needed to start the game
  ...
  self.listPlayers = function () {
    const activePlayers = _.filter(self.players, function (player) { return player.isActive; });
  ...
  self.findAndRemoveIfPlaying = function (nick) {
    const player = self.getPlayer({nick: nick});
