        ...
        validator.validate(data, schema, function (errors) {
            if (errors) {
                ...
                data.forEach(function (card) {
                  if (card.type.toLowerCase() === 'question') {
                    updateOrCreateInstance(models.Card,
                      {where: {text: card.value}},
                      {text: card.value, times_played: 0, question: true},
                      null
                    );
                  } else if (card.type.toLowerCase() === 'answer') {
                    updateOrCreateInstance(models.Card,
                      {where: {text: card.value}},
                      {text: card.value, times_played: 0, question: false},
                      null
                    );
                  }
                });
...
var updateOrCreateInstance = function(model, query, createFields, updateFields) {
